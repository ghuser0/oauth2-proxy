	...
	testCases := []struct {
		...
		},
		{
			...
			sessionState: &SessionState{
				...
				SignOutKeys:       []string{"signoutkey-a", "signoutkey-b"},
			},
			expected: "Session{email:email@email.email user:some.user PreferredUsername:preferred.user token:true id_token:true created:2000-01-01 00:00:00 +0000 UTC expires:2000-01-01 01:00:00 +0000 UTC refresh_token:true sign_out_keys:2}",
		},
		{
...
func TestEncodeAndDecodeSessionState(t *testing.T) {
	...
	testCases := map[string]SessionState{
		"Full session": {
			...
			SignOutKeys:       []string{"sign-out-key-a", "sign-out-key-b"},
		},
		"No Groups or SignOutKeys": {
			Email:             "username@example.com",
			User:              "username",
			PreferredUsername: "preferred.username",
			AccessToken:       "AccessToken.12349871293847fdsaihf9238h4f91h8fr.1349f831y98fd7",
